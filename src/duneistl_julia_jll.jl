...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import duneistl_julia_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"duneistl_julia"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("duneistl_julia_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "duneistl_julia_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"duneistl_julia", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "duneistl_julia_jll", "src", "duneistl_julia_jll.jl"))
        @info("duneistl_julia_ll dev'ed out to /home/yiminliu/.julia/dev/duneistl_julia_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("228c9359-a2f9-5b9c-be5b-bc96acd46855"))
platforms = [Pkg.Artifacts.unpack_platform(e, "duneistl_julia", artifacts_toml) for e in artifacts["duneistl_julia"]]
...
if best_platform === nothing
    @debug("Unable to load duneistl_julia; unsupported platform $(triplet(platform_key_abi()))")
else
